...
namespace tflite
{
  namespace
  {
    ...
    TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node)
    {
      ...
      case kTfLiteInt8:
      {
        TfLiteStatus status = EvalQuantizedInt8(context, node, data, input, filter, bias,
                                                output);
        TflmTimer::measureTime(CMSIS_FULLY_CONNECTED);
        return status;
      }
    ...
    TfLiteStatus EvalInt8(TfLiteContext *context, TfLiteNode *node)
    {
      ...
      TfLiteStatus status = EvalQuantizedInt8(context, node, data, input, filter, bias, output);
      TflmTimer::measureTime(CMSIS_FULLY_CONNECTED);
      return status;
    }
