...
namespace tflite
{
  namespace
  {
    ...
    TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node)
    {
      ...
      case kTfLiteInt8:
      {
        TfLiteStatus status = EvalQuantizedPerChannel(context, node, params, data, input, filter,
                                                      bias, output);
      ...
      case kTfLiteInt16:
      {
        TfLiteStatus status1 = EvalQuantizedPerChannel16x8(context, node, params, data, input,
                                                           filter, bias, output);
        TflmTimer::measureTime(CMSIS_CONV);
        return status1;
        break;
