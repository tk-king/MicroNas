...
class RandomAugment(object):
    def __init__(self, transformation_count, random_augmentation_config, max_aug):
        ...
        self.all_selected_transformations = list(random_augmentation_config.keys())
        print(" ---------------------")
        print(self.all_selected_transformations)
        print(self.random_augmentation_config)
    ...
    def __call__(self, org_sample_x):
        ...
        #nr = min(len(random_augmentation_config.keys(), nr)
        ...
        for key in transformations:
            ...
            if len(keysplit)>2:
                key_ = "_".join(keysplit[:-1])
            else:
                key_ = keysplit[0]
            #print(key_,self.random_augmentation_config[key])
            sample_x = self.all_transformations_dict[key_](sample_x,**self.random_augmentation_config[key])
        return sample_x
    ...
    @staticmethod
    def moving_average(x: np.ndarray, ws = 3):
        if len(x.shape)==2:
            x = np.expand_dims(x, axis=0)
        batch = x.shape[0]
        seq_l = x.shape[1]
        ...
        for i in range(batch):
            df = pd.DataFrame(x[i])
            avaraged_df = df.rolling(window =ws).mean()
