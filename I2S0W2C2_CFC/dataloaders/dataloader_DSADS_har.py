...
class DSADS_HAR_DATA(BASE_DATA):
    ...
    def load_all_the_data(self, root_path):
        ...
        df_dict = {}
        action_list = os.listdir(root_path)
        action_list = [file for file in action_list if "checkpoints" not in file]
        for action in action_list:
            action_name = action[1:]
            user_list = os.listdir(os.path.join(root_path,action))
            user_list = [file for file in user_list if "checkpoints" not in file]
            for user in user_list:
                user_name = user[1:]
                seg_list = os.listdir(os.path.join(root_path,action,user))
                seg_list = [file for file in seg_list if "checkpoints" not in file]
                for seg in seg_list:
                    seg_name = seg[1:3]
