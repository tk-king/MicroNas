...
from I2S0W2C2_CFC.dataloaders.utils import Normalizer,components_selection_one_signal,mag_3_signals,PrepareWavelets,FiltersExtention
    ...
    def update_train_val_test_keys(self):
        ...
        if self.exp_mode in ["Given", "LOCV"]:
            if self.exp_mode == "LOCV":
                ...
                print("Train Test Experiment : The {} Part as the test".format(self.index_of_cv+1))
                self.test_keys =  self.given_test_keys
                self.train_keys = self.given_train_keys
                self.index_of_cv = self.index_of_cv + 1      
