...
class Exp(object):
    def __init__(self, args):
        ...
        if self.args.model_type in ["tinyhar","attend","mcnn","deepconvlstm_attn","sahar","deepconvlstm","mixermlp","alae","deepsense","globalfusion","aele","visionmixer"]:
            macs, params = get_model_complexity_info(self.model , 
                                                     (1, self.args.windowsize, self.args.c_in), 
    ...
    def get_setting_name(self):
        if self.args.model_type in ["deepconvlstm", "deepconvlstm_attn", "mcnn", "attend", "sahar", "tinyhar", "deepsense","globalfusion","aele","visionmixer"]:
            setting = "model_{}_data_{}_seed_{}_differencing_{}_Seperation_{}_magnitude_{}_Mixup_{}_RandomAug_{}_oration_{}_Mixupargmax_{}_randomconfig_{}".format(self.args.model_type,
                                                                                                                         self.args.data_name,
                                                                                                                         self.args.seed,
                                                                                                                         ...
                                                                                                                         self.args.random_augmentation_prob,
                                                                                                                                    self.args.mixer_oration,
                                                                                                                                                   self.args.mixup_argmax,
                                                                                                                                                   self.args.random_augmentation_config_path

                                                                                                                         )
            return setting
        elif self.args.model_type == "mixermlp":
            setting = "model_{}_data_{}_Feature_{}_expansion_{}_seglength_{}_Numberlayer_{}_earlyfusion_{}_seed_{}_differencing_{}_representation_{}_RandomAug_{}_merge_{}_oration_{}_skip_{}".format(self.args.model_type,
                                                                                                                                                                  self.args.data_name,
                                                                                                                                                                self.args.mixer_mlp_feature_mum,
                                                                                                                                                                  self.args.mixer_mlp_expansion_factor,
                                                                                                                                                                self.args.fft_segments_length,
                                                                                                                                                                                 self.args.fft_mixer_layer_nr,
                                                                                                                                                                                                self.args.mixer_fuse_early,
                                                                                                                                                                self.args.seed,
                                                                                                                                                                self.args.difference,
                                                                                                                                                                self.args.mixer_representation,
                                                                                                                                                                          self.args.random_augmentation_prob,
                                                                                                                                                                                   self.args.mixer_merge,
                                                                                                                                                                                              self.args.mixer_oration,
                                                                                                                                                                                                 self.args.residual_flag
    ...
    def train(self):
        ...
        for iter in range(num_of_cv):
            ...
            if skip_train:
                ...
                epoch_log.write("Number of Parameters : {} -------- Model Complexity : {:<8} ------------ Number of parameters 2: {:<8} ".format(self.model_size, self.macs, self.params))
                epoch_log.write("\n")
                ...
                # ---------- save onnx file-------------
                # Export the model
                # x_input = torch.randn(1, 1, self.args.input_length, self.args.c_in).double().to(self.device)
                # torch.onnx.export(
                #     self.model,               # model being run
                #     x_input,                         # model input (or a tuple for multiple inputs)
                #    cv_path+'/'+"final_best_vali.onnx",   # where to save the model (can be a file or file-like object)
